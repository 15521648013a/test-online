...
function aa(style,elem,isReNew=false,location='',myanswer='',score,truesore=0){
    ...
    var str = '<div class="question-each" style="border-bottom: 1px solid #7FFFD4; ">'
      ...
      // console.log(json);
       ...
       j=65;
        ...
        str +="</div>"+"正确答案："+elem['answer']+"<br/>考生的回答："+myanswer+"<br/>该题分值："+truesore+"<br/>得分： "+score;
        str+='<input type="hidden"  name='+location+' value='+score+'></div>';
       ...
       // num[style]=++qIndex; 
        ...
        //清空questionIds[1]，接收新的
          ...
          var str = '<div class="question-each" style="border-bottom: 1px solid #7FFFD4; ">'
                   ...
                   + '<div class="option "> ';        
     str +="  </div> ";   
     str +="<div>"+"参考答案："+elem['answer']+"</div> <div> 考生的回答："+myanswer+"<br/>该题分值："+truesore+"<br/>得分： "+score+"</div></div> ";          
     //  $("#fill").append(str);
     return str;
       num[style]++;
 ...
 var str = '<div class="question-each" contenteditable="false" name="question-each" style="border-bottom: 1px solid #7FFFD4; ">'
                + '<div class="question-name" style="display:inline-block;"><span class="index">'+(qIndex)+'</span>.'+elem['title']+'</div>'
                + '<div class="option "> ';        
  str +=' <p>考生的回答：<textarea contenteditable="true" style="BORDER-TOP-STYLE: none; BORDER-RIGHT-STYLE: none; BORDER-LEFT-STYLE: none; width:100%;    min-height: 200px;" > '+myanswer+'</textarea></p> </div> </div><p><br></p>';             
  str+='老师评分：'+score;
  str +="<div>"+"参考答案："+elem['answer']+"</div> ";   
    ...
    $.each(questions,function(index1,elem){  
        ...
        $.each(elem['questions'],function(index2,k){
          ...
          if(!k["Qstyle"]){
            ...
            var strAnswer='';var score=0;var truesore=0;
            if(questions.hasOwnProperty(index1)){
            if(questions[index1]['questions'].hasOwnProperty(index2)){
             // alert("存在");
              strAnswer =typeof(answer[index1]["'questions'"][index2])=="undefined"?'':answer[index1]["'questions'"][index2];
              //console.log(scores[index1]['questions']);
              score = scores[index1]["'questions'"][index2];
              truesore = typeof(truescores[index1]["questions"][index2])=="undefined"?0:truescores[index1]["questions"][index2];
              if($.isArray(score)){
