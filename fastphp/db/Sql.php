...
class Sql
{
     ...
     * 联合查询，使用方式：
     *
     * $this->join(table , condition=array('aid','bid')  )
     *a join inner btable b on a.id = b.id
     * @param array $order 排序条件
     * @return $this
     */
    public function join($style='',$tableA="",$tableB="" ,$condition1="",$condition2=""){
        if($tableA=='') $tableA = $this->table;//当前主表
        $this->join .= " $style join {$tableB}  ";
        $this->join .="on  {$tableA}.{$condition1} = {$tableB}.{$condition2}";
        return $this;
    }
    // 查询所有
